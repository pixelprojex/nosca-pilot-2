...
(async () => {
  ...
  try {
    ...
    /* ---------- (b) the coach: who is asking, accept, decline ---------- */
    {
      ...
      check("(b) the request landed under the coach's catch-up on opening", (await page.locator('[data-tour="catchup-list"]').count()) === 1 && cu.includes("Eoin Walsh asked to join you"), cu.slice(0, 200));
      ...
      check("(b) Today says who asked to join", (await strip.count()) === 1 && M.norm(await strip.innerText()).includes("Eoin Walsh asked to join you"), t0.slice(0, 200));
      ...
      check("(b) Today now names the new asker", (await page.locator('[data-tour="today-requests"]').count()) === 1 && t5.includes("Aoife Nolan asked to join you"), t5.slice(0, 200));
