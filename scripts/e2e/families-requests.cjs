...
(async () => {
  ...
  try {
    ...
    /* ---------- (b) the coach: who is asking, accept, decline ---------- */
    {
      ...
      check("(b) Today says how many asked to join", (await strip.count()) === 1 && /Join requests/i.test(M.norm(await strip.innerText())), t0.slice(0, 200));
      ...
      check("(b) a junior's file names the adult in her family messages go to", t4.includes("Parent · Orla Kelly"), t4.slice(0, 200));
      ...
      check("(b) Today still counts the one asking", (await page.locator('[data-tour="today-requests"]').count()) === 1 && /Join requests/i.test(M.norm(await page.locator('[data-tour="today-requests"]').innerText())), t5.slice(0, 200));
    ...
    let famCode = null;
    {
      ...
      await click(page, "Start", 1800);
      const cf = rpc("create_family")[0]; const fam = Object.values(db.families).find((f) => f.created_by === IDS.adult);
      famCode = fam && fam.code;
      check("(d) Start calls create_family with the name and the caller joins it", !!cf && cf.args.p_name === "The Murphys" && !!fam && db.profiles[IDS.adult].family_id === fam.id, JSON.stringify({ call: cf && cf.args, code: famCode }));
    ...
    /* ---------- (e) a second adult joins with the code, then leaves ---------- */
    {
      ...
      check("(e) Leave calls leave_family; the family stays for the one still in it", rpc("leave_family").length === 1 && !db.profiles[IDS.dara].family_id && !!db.profiles[IDS.adult].family_id && !!db.families[db.profiles[IDS.adult].family_id], JSON.stringify({ dara: db.profiles[IDS.dara].family_id, cian: db.profiles[IDS.adult].family_id }));
    ...
    /* ---------- (f) a parent: the dashboard, booking for the child, writing to her coach ---------- */
    {
      ...
      check("(f) the request sheet names the child, not the parent", /· Saoirse/.test(sheetText) && sheetText.includes("Request for Saoirse"), sheetText.slice(-200));
    ...
    /* ---------- (g) the junior's view of the family ---------- */
    {
      ...
      /* RowHead sets the section head in caps, so innerText reads COMING UP */
      check("(g) …with no Next up card and no sentence of explanation", !/Next up/i.test(t1) && !t1.includes("The adults here can see your lessons") && /Coming up/i.test(t1), t1.slice(0, 240));
      await tap(page, '[data-tour="family-settings"]', 900);
      const t2 = await leak("junior family screen"); await shot("27-junior-family-screen");
      check("(g) the junior's Family screen has no code, no rename and no Leave", !t2.includes("Family code") && !t2.includes("KEL7Y2") && !t2.includes("Leave the family") && !t2.includes("Name the family") && t2.includes("Members") && !/\bLeave\b/.test(t2) && !t2.includes("Family name") && !t2.includes("Ask an adult"), t2.slice(0, 240));
      await ctx.close();
    }
  } catch (e) {
