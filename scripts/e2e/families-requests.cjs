...
(async () => {
  ...
  try {
    ...
    /* ---------- (b) the coach: who is asking, accept, decline ---------- */
    {
      ...
      check("(b) Today says how many asked to join", (await strip.count()) === 1 && /Wanting to join/i.test(M.norm(await strip.innerText())), t0.slice(0, 200));
      ...
      check("(b) Today still counts the one asking", (await page.locator('[data-tour="today-requests"]').count()) === 1 && /Wanting to join/i.test(M.norm(await page.locator('[data-tour="today-requests"]').innerText())), t5.slice(0, 200));
