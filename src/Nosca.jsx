          ...
          <Group title={cfg.nouns}>
            {people.slice(0, 4).map((r, i) => (
              <Line key={r.name} i={i} Ico={User} label={r.name}
                    sub={`${r.lessons ?? fileFor(r.name).done} ${tr("lessons")}`}
    ...
    <Screen bare right={right}>
      ...
      {!ready ? (
        ...
        <div className="px-6 pb-4" style={{ borderTop: `0.5px solid ${HAIR(t.ink, 0.14)}` }}>
          {shown.map((l, i) => (
            <button key={l.id} onClick={() => { haptic(8); soft(); push(`lesson:${l.id}`); }}
                    className="w-full flex items-center gap-4 text-left active:opacity-50"
...
function PlayerLesson({ cfg, conn, lessons, go, push, pop, fresh, saved, toggleSave, minimise, attendance, lessonId, mediaFor, onDownload, onRate }) {
  ...
  return (
    <SwipeBack onBack={pop}>
      <Screen bare onBack={pop}
              right={live
                ? <IconBtn C={Download} label={tr("Download lesson log")} onOpen={() => { haptic(9); onDownload && base && onDownload(base, items); }} />
                : <IconBtn C={Download} label={tr("Save offline")} onOpen={() => { haptic(9); toggleSave(id); }} />}>
...
function Wizard({ cfg, sport, prefill, groups, captured, setCaptured, onAnnotate, showGuide, onDismissGuide, onPublish, onCancel, livePlayers, askReview = true, lessonCounts, onSaveDrill, live }) {
        ...
        <div className="flex-1 overflow-y-auto pt-8">
          ...
          {step === 3 && (<div className="px-6">
            ...
            {rec === "idle" && !(live ? voice : note) && !(live && !cap.supported) && (
              <button onClick={() => { if (live) { startVoice(); return; } haptic(14); setRec("recording"); setSecs(0); }} className="w-full flex flex-col items-center justify-center gap-3 active:opacity-70"
          ...
          {step === 4 && (<div className="px-6">
            ...
            <div className="grid gap-1.5 mb-5" style={{ gridTemplateColumns: `repeat(${live ? 4 : 5}, minmax(0, 1fr))` }}>
              {(live ? [
  ...
  return (
    <Screen title={tr("Messages")} right={right}
      ...
      <div className="px-6 pb-2"><Card>{list.length === 0 ? (
        ...
        <button key={c.id || c.name} onClick={() => { haptic(6); push("thread:" + (c.id || c.name)); }} className="w-full flex items-center gap-3.5 px-5 text-left active:opacity-50" style={{ minHeight: 74, borderBottom: i === list.length - 1 ? "none" : `1px solid ${t.hair}` }}>
      ...
      <div className="flex flex-col h-full" style={{ background: t.page }}>
        ...
        <div className="shrink-0 px-3 pt-2 pb-3" style={{ background: `${t.surface}F2`, backdropFilter: "blur(18px)", borderTop: `1px solid ${t.hair}` }}>
          <div className="flex items-end gap-2">
            ...
            <button onClick={send} disabled={!draft.trim() || sending} className="rounded-full flex items-center justify-center shrink-0 active:opacity-60 disabled:opacity-25" style={{ width: 40, height: 40, background: t.accent }} aria-label={tr("Send")}><Send size={17} color={t.onAccent} /></button>
          </div>
        </div>
      </div>
    </SwipeBack>
    ...
    <SwipeBack onBack={pop}>
      <Screen title={tr("You")} onBack={pop}>
        ...
        {role === "coach" ? (<><Eyebrow>{tr("Coaching")}</Eyebrow><div className="px-6 mb-6"><Card>
          ...
          <Row label={tr("Lesson logs")} sub={tr("Save any lesson as a file")} chevron icon={<I C={Download} />} onToggle={() => push("lessonLogs")} />
...
function SearchScreen({ role, cfg, library, tips, pop, go, push, lessons: given }) {
  ...
  const lessons = term ? (given || hadLessons(cfg)).filter((l) => hit(l.focus) || (l.subs || []).some(hit) || hit(l.who)) : [];
...
export default function Nosca({ demo: demoProp, account, onSignOut, data, onJoinCoach } = {}) {
  ...
  const archive = data ? (data.lessons || []).map((l) => ({ ...l, who: l.who || "—" })) : freshAccount ? [] : buildArchive(cfg);
