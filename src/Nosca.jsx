...
/* Whether this tree is a real, signed-in account. While true, every seed
   ...
   surface invented history, whether or not it remembered to ask for the
   live data. It travels by context rather than a module flag because the
...
const hadLessons = (cfg, live) => (live ? [] : (cfg?.lessons || []));
...
const fmtIsoDay = (iso) => {
  ...
  return `${DAY_NAMES[(dt.getDay() + 6) % 7].slice(0, 3)} ${d} ${MONTHS_FULL[m - 1].slice(0, 3)}`;
};
const MONTH_SHAPE = [
...
const LEGAL = {
  terms: { title: "Terms of Service", updated: "Last updated 1 July 2026", body: [
    ...
    ["Money", `${BRAND} does not process lesson fees — what a coach charges and how they collect it stays between them and their players.`],
  ...
  privacy: { title: "Privacy Policy", updated: "Last updated 1 July 2026", body: [
    ...
    ["What we collect", "Your name, email, mobile and date of birth; lessons, video, audio, drills, notes and messages; and basic device and usage information."],
...
function EventCard({ event, sport, onPress, delay = 0 }) {
  ...
  const near = !past && event.days <= 7;
  const soon = !past && event.days <= 21;
    ...
    <Tile accent={tone} className="px-5 py-4 mb-2.5" onPress={onPress} delay={delay}>
      <div className="flex items-center gap-3.5">
        <span className="rounded-2xl flex flex-col items-center justify-center shrink-0"
          ...
          <span style={{ fontFamily: display, fontSize: past ? 13 : 19, lineHeight: 1, letterSpacing: "-0.02em",
                         color: past ? t.faint : tone || t.ink }}>{past ? tr("Done") : event.days}</span>
          {!past && <span className="uppercase" style={{ fontFamily: ui, fontSize: 7.5, letterSpacing: "0.14em", color: t.faint }}>
...
function LessonPeek({ booking, duration, sport, cfg, agreed, past, comps = [], lessonCount,
  ...
  return (
    <>
      ...
      <button data-tour="peek-log" onClick={() => { hapticCommit(); onLog(); }}
...
/* ==================================================================
   ...
   looked nothing like it. Now each step renders the product itself —
   a second <Nosca> given a `showcase` state, seeded, made inert, and
   scaled into a card — and rings the real control by finding its
...
const TOUR = {
  coach: [
    ...
    { area: "Log a lesson", title: "Their level", body: "Already set to the level they are on. Tap it only when it has changed.", path: "Log a lesson", target: "wiz-stage", state: { stack: ["log"], prefill: TOUR_PREFILL, wizardView: "main" } },
    { area: "Log a lesson", title: "Video, photo, voice", body: "One tap opens the camera. Anything you captured mid-lesson is already on it — swipe left to take one off.", path: "Log a lesson", target: "wiz-media", state: { stack: ["log"], prefill: TOUR_PREFILL, wizardView: "main" } },
    { area: "Log a lesson", title: "Drills and the tip", body: "What they practise until next time, and the one line they remember.", path: "Log a lesson", target: "wiz-drills", state: { stack: ["log"], prefill: TOUR_PREFILL, wizardView: "main" } },
    ...
    { area: "Roster", title: "A player's file", body: "Message, a tip, drills or Log, then their lessons newest first.", path: "Roster → name", target: "player-lessons", state: { stack: ["roster", "player:Marcus Tran"] } },
  ...
  player: [
    ...
    { area: "Family", title: "Family", body: "Optional. Start or join one so a parent can follow a young player. It becomes its own tab.", path: "You → Family", target: "settings-dashboard", state: { stack: ["home", "you"] } },
    ...
    { area: "You", title: "Your profile", body: "Photo, details and password.", path: "Header → avatar → your name", target: "settings-profile", state: { stack: ["home", "you"] } },
    { area: "You", title: "This walkthrough", body: "Come back to it any time from here.", path: "You → Walkthrough", target: "settings-tour", state: { stack: ["home", "you"] } },
  ],
  parent: [
    { area: "Family", title: "Your family", body: "One line per young player. Tap to open them.", path: "Tab bar → Family", target: "family-kid", state: { stack: ["family"] } },
    { area: "Family", title: "Book and message", body: "Book into their coach's hours, or write to the coach.", path: "Family → a young player", target: "kid-book", state: { stack: ["family", "familyKid:first"] } },
    ...
    { area: "You", title: "Your profile", body: "Photo, details and password.", path: "Header → avatar → your name", target: "settings-profile", state: { stack: ["family", "you"] } },
    { area: "You", title: "This walkthrough", body: "Come back to it any time from here.", path: "You → Walkthrough", target: "settings-tour", state: { stack: ["family", "you"] } },
  ],
  juvenile: [
    ...
    { area: "Family", title: "Family", body: "Who's in your family. They book and message your coach for you.", path: "You → Family", target: "settings-dashboard", state: { stack: ["home", "you"] } },
    { area: "You", title: "This walkthrough", body: "Come back to it any time from here.", path: "You → Walkthrough", target: "settings-tour", state: { stack: ["home", "you"] } },
  ],
};
...
function showcasePersona(sc) {
  const coachOf = (s) => ({ sport: s, coach: COACHES[s][0].name, club: COACHES[s][0].club, seeded: true });
  const me = { id: 1, name: "Marcus Tran", age: null };
  const kid = { id: 2, name: "Ellie Tran", age: 14, turns18: "March 2030" };
  const booking = (connId) => [{ m: 7, d: 31, time: "4:30 pm", connId }];
  if (sc.role === "player" && sc.juvenile) {
    const id = sc.sport === "tennis" ? 2 : 1;
    return { profiles: sc.profiles || [{ ...kid, id }], activeProfileId: id,
             conns: [{ id, profileId: id, ...coachOf(sc.sport) }], activeId: id, bookings: booking(id) };
  }
  if (sc.role === "player" && sc.parent) {
    return { profiles: sc.profiles || [me, kid], activeProfileId: 1,
             conns: [{ id: 1, profileId: 1, ...coachOf(sc.sport) }, { id: 2, profileId: 2, ...coachOf("tennis") }], activeId: 1, bookings: booking(1) };
  }
  return { profiles: sc.profiles || [me], activeProfileId: 1,
           conns: [{ id: 1, profileId: 1, ...coachOf(sc.sport) }], activeId: 1, bookings: booking(1) };
}

/* Scroll the nearest scrollable ancestor inside the frame so `el` sits
   in the middle. Not scrollIntoView: that would also nudge the scaled
   card and the page around it. Rects are screen pixels; scrollTop is
   the element's own, so the delta is divided by the frame's scale. */
function centreInFrame(el, root, scale) {
  ...
  while (p && p !== root) {
    ...
    if (/(auto|scroll)/.test(cs.overflowY) && p.scrollHeight > p.clientHeight + 1) {
      ...
      p.scrollTop += ((er.top + er.height / 2) - (pr.top + pr.height / 2)) / scale;
      return true;
    }
    p = p.parentElement;
  }
...
/* Something in the frame still moving? Finite CSS animations and
   transitions on the element, on anything above it inside the frame, or
   on anything inside it. Infinite ones (a breathing bar, a pulsing dot)
   never end, so they don't count. */
function frameSettled(el, root) {
  if (typeof el.getAnimations !== "function") return true;
  const running = (node, opts) => node.getAnimations(opts).some((a) => {
  ...
  if (running(el, { subtree: true })) return false;
  for (let p = el.parentElement; p && p !== root; p = p.parentElement) if (running(p)) return false;
  return true;
}

/* The showcased app, insulated from the walkthrough's own re-renders:
   a fresh element tree each time it re-rendered would restart every
   entrance animation and move the very thing being measured. */
const ShowcaseApp = React.memo(function ShowcaseApp({ showcase }) {
  return <Nosca showcase={showcase} />;
});
...
function Walkthrough({ role, juvenile, isParent, sport, onClose }) {
  ...
  /* A coach's seeds exist for every sport; a player's for golf and tennis. */
  const scSport = key === "coach" ? (sport || "golf") : sport === "tennis" ? "tennis" : "golf";
  const showcase = useMemo(() => ({ role: key === "coach" ? "coach" : "player", juvenile: key === "juvenile", parent: key === "parent",
                                    sport: scSport, ...step.state }), [key, scSport, step]);

  const stageRef = useRef(null);   // the space the card may fill
  const frameRef = useRef(null);   // the 390×780 logical frame, scaled
  ...
  useEffect(() => {
    ...
    const fit = () => {
      ...
      setK(Math.max(0.2, Math.min((r.width - 12) / FRAME_W, (r.height - 12) / FRAME_H)));
      setFits((n) => n + 1);
    };
    fit();
  ...
  /* Measure the target once the screen has settled. Skeletons last
     560ms, sheets slide for 340ms and rows settle in with delays, so
     this polls by animation frame and asks the frame whether anything
     that could move the target is still animating. A quiet box that
     holds for two frames is drawn; if something later moves it, it is
     redrawn, for up to ~2.6s. No target → no ring. */
  useEffect(() => {
    setRing(null);
    const root = frameRef.current; if (!root) return;
    let raf = 0, stop = false, scrolled = false, seen = null, still = 0, drawn = null;
    ...
    const look = () => {
      ...
      const scale = fr.width / FRAME_W || 1;
      const age = performance.now() - t0;
      if (el) {
        const er = el.getBoundingClientRect();
        const box = { x: (er.left - fr.left) / scale, y: (er.top - fr.top) / scale, w: er.width / scale, h: er.height / scale };
        const off = box.y < 0 || box.y + box.h > FRAME_H;
        const quiet = frameSettled(el, root);
        if (!quiet) { still = 0; seen = box; }
        else if (off && !scrolled) { scrolled = true; centreInFrame(el, root, scale); seen = null; still = 0; }
        else if (box.w > 0 && box.h > 0) {
          still = same(box, seen) ? still + 1 : 0;
          seen = box;
          if (still >= 2 && !same(box, drawn)) { drawn = box; setRing(box); }
        }
      }
      if (age < 2600) raf = requestAnimationFrame(look);
      else if (!drawn && seen && seen.w > 0) setRing(seen);
      else if (!drawn && import.meta.env.DEV) console.warn(`[walkthrough] no target "${step.target}" for ${key} step ${i + 1}`);
    };
  ...
  return (
    <div className="absolute inset-0 flex flex-col" style={{ zIndex: 72, background: t.page,
        ...
        <div key={`s${i}`} className="relative overflow-hidden"
          ...
          <div ref={frameRef} style={{ width: FRAME_W, height: FRAME_H, transform: `scale(${k})`, transformOrigin: "top left", position: "relative" }}>
            <div inert="" aria-hidden="true" style={{ width: FRAME_W, height: FRAME_H, pointerEvents: "none" }}>
              <ShowcaseApp showcase={showcase} />
            </div>
            {ring && (
              <div data-tour-ring="" data-for={step.target} aria-hidden="true"
                   style={{ position: "absolute", left: ring.x - 5, top: ring.y - 5, width: ring.w + 10, height: ring.h + 10,
                            borderRadius: 16, border: `2.5px solid ${t.accent}`, ["--ring"]: `${t.accent}66`,
                            boxSizing: "border-box", pointerEvents: "none", zIndex: 100,
      ...
      <div key={`t${i}`} className="text-center px-8 shrink-0" style={{ animation: "slideFrom 460ms cubic-bezier(.22,1,.36,1) 80ms both" }}>
        <span className="block uppercase" style={{ ...TYPE.eyebrow, color: t.accent }}>{tr(step.area)}</span>
        <h2 className="mt-2" data-tour-title="" style={{ fontFamily: display, fontSize: 24, lineHeight: 1.12, letterSpacing: "-0.032em", color: t.ink }}>{tr(step.title)}</h2>
        <p className="mt-2" style={{ fontFamily: ui, fontSize: 14.5, lineHeight: 1.5, color: t.sub }}>{tr(step.body)}</p>
        <p className="mt-2" style={{ ...TYPE.caption, color: t.faint }}>{step.path}</p>
      </div>
...
function AtRisk({ list, onMessage, pop }) {
  const t = useT();
  return (
    <SwipeBack onBack={pop}>
      <Screen title={tr("Drifting")} onBack={pop}
        ...
        <div className="px-6 pb-2">
          {list.length === 0 ? (
            ...
            /* rows, one action: the number says it, the sentence is spare */
            <div style={{ borderTop: `0.5px solid ${HAIR(t.ink, 0.1)}` }}>
              {list.map((x) => (
                <div key={x.id || x.name} className="flex items-center gap-3.5" style={{ minHeight: 66, borderBottom: `0.5px solid ${HAIR(t.ink, 0.1)}` }}>
                  <Avatar name={x.name} size={40} />
                  <span className="flex-1 min-w-0">
                    <span className="block truncate" style={{ ...TYPE.body, color: t.ink }}>{x.name}</span>
                    <span className="block mt-0.5 truncate" style={{ ...TYPE.caption, color: x.weight >= 3 ? CAUTION : t.faint }}>
                      {x.gap != null ? `${x.gap} ${tr("days")}` : tr(x.why)}
                    </span>
                  </span>
                  <button onClick={() => { hapticCommit(); onMessage(x.id || x.name); }} className="shrink-0 px-3 active:opacity-60"
                          style={{ minHeight: 40, ...TYPE.small, fontWeight: 600, color: t.accent }}>{tr("Message")}</button>
                </div>
              ))}
            </div>
          )}
      ...
      <Screen title={tr("This month")} onBack={pop} meta={tr("How everyone's getting on")}>
        <div className="px-6 pb-2">
          {kids.map((k, i) => {
            ...
            return (
              <Tile key={k.id} tour="digest-tile" className="px-5 py-5 mb-3" delay={i * 80}>
      ...
      <div className="mt-2.5">
        <TileGrid cols={2}>
          {QUICK_ORDER.map((id) => {
            ...
            return <ActTile key={id} tour={`quick-${id}`} h={78} Icon={A.Ico} label={tr(A.label)}
...
function CoachProfile({ coachName, sport, reviewSummary, myReview, onSubmitReview, onMessage, onLeaveCoach, juvenile, pop }) {
  const t = useT();
  ...
  const [leaving, setLeaving] = useState(false);
  const first = (coachName || "").split(" ")[0] || tr("your coach");

  /* "Saved" only once the row is actually there */
  const submit = async () => {
    if (!rating || busy) return;
    hapticCommit(); soft();
    setBusy(true); setErr("");
    const res = await onSubmitReview(rating, comment.trim());
    setBusy(false);
    ...
    <SwipeBack onBack={pop}>
      <Screen title={coachName || tr("Your coach")} onBack={pop}
        ...
        <div className="px-6">
          ...
          {!juvenile && (
            <button data-tour="coach-message" onClick={() => { haptic(8); soft(); onMessage(); }}
          ...
          {!juvenile && (
            <div className="pt-2" data-tour="coach-review" style={{ borderTop: `1px solid ${HAIR(t.ink, 0.14)}` }}>
              ...
              {err && <p className="mt-3" style={{ ...TYPE.caption, color: DANGER }}>{err}</p>}
              <button onClick={submit} disabled={busy || !rating || (sent && comment === (myReview?.comment || "") && rating === myReview?.rating)}
                ...
                {busy ? "…" : sent ? tr("Saved") : myReview ? tr("Update review") : tr("Submit review")}
              </button>
            </div>
          )}

          {/* The way out. Without it a wrong six-character code bonded a
              player to a coach for the life of the account. */}
          {!juvenile && onLeaveCoach && (
            <div className="mt-8 pt-2" style={{ borderTop: `1px solid ${HAIR(t.ink, 0.14)}` }}>
              {leaving ? (
                <div className="py-4">
                  <p className="mb-3" style={{ ...TYPE.small, lineHeight: 1.55, color: t.sub }}>
                    {tr("Your lessons stay with them. You can join another coach after.")}
                  </p>
                  {leaveErr && <p className="mb-3" style={{ ...TYPE.caption, color: DANGER }}>{leaveErr}</p>}
                  <div className="flex gap-2">
                    <button onClick={() => { haptic(6); setLeaving(false); }} className="flex-1 active:opacity-60"
                            style={{ minHeight: 46, borderRadius: R.control, border: `1px solid ${t.hair}`, ...TYPE.small, fontWeight: 600, color: t.sub }}>{tr("Stay")}</button>
                    <button onClick={async () => { hapticWarn(); setLeaveBusy(true); const r = await onLeaveCoach(); setLeaveBusy(false); if (r && r.error) { setLeaveErr(r.error.message); return; } }}
                            className="flex-1 active:opacity-80"
                            style={{ minHeight: 46, borderRadius: R.control, background: DANGER, ...TYPE.small, fontWeight: 600, color: "#fff" }}>{leaveBusy ? "…" : `${tr("Leave")} ${first}`}</button>
                  </div>
                </div>
              ) : (
                <button data-tour="coach-leave" onClick={() => { haptic(8); setLeaving(true); }} className="w-full text-left py-4 active:opacity-50"
                        style={{ ...TYPE.body, color: DANGER }}>{tr("Leave")} {first}</button>
              )}
            </div>
          )}
        </div>
...
function ConfirmPassword({ title, detail, actionLabel, onConfirm, close }) {
  ...
  const go = async () => {
    ...
    else { hapticCommit(); if (!(res && res.next)) close(); }   // a confirm that opens the next sheet must not close it again
  };
...
/* FAMILY — the settings side.
   ...
   is set up. */
function FamilyScreen({ family, isJunior, onCreate, onJoin, onLeave, onRename, lookup, say, pop }) {
  const t = useT();
  const [name, setName] = useState("");
  const [busy, setBusy] = useState(false);
  const [err, setErr] = useState("");
...
/* bookings in the order they happen: "1:00 pm" sorts after "9:00 am"
   only if the clock is read, not the string */
const byWhen = (x, y) => (x.date === y.date ? (parseTime(x.time) ?? 0) - (parseTime(y.time) ?? 0) : x.date < y.date ? -1 : 1);
function FamilyHome({ family, isJunior, dependants = [], lessons = [], drills = [], bookings = [], onOpenKid, onSettings, onSetUp, right, todayIso }) {
  const t = useT();
  const ready = useLoad();
    ...
    return (
      <Screen title={tr("Family")} right={right}>
        <div className="px-6 flex flex-col items-center text-center pt-6">
          ...
          <p className="mt-2 mb-7" style={{ ...TYPE.small, lineHeight: 1.55, color: t.sub, maxWidth: 300 }}>{tr("Start one and share its code, or join with a code you were given.")}</p>
  ...
  const next = isJunior ? null : live[0] || null;
  const rest = isJunior ? live.slice(0, 6) : live.slice(1, 6);
  ...
  const KidRow = ({ k, i, last }) => {
    ...
    return (
      <button data-tour={i === 0 ? "family-kid" : undefined} onClick={() => { haptic(7); soft(); onOpenKid && onOpenKid(k); }}
              className="w-full flex items-center gap-3.5 py-3 text-left active:opacity-50"
              style={{ borderBottom: last ? "none" : `0.5px solid ${HAIR(t.ink, 0.12)}` }}>
        <Avatar name={k.name} size={42} src={avatarUrl(k.avatarPath)} />
        <span className="flex-1 min-w-0">
          <span className="block truncate" style={{ ...TYPE.body, fontWeight: 600, color: t.ink }}>{first(k.name)}</span>
          <span className="block mt-0.5 truncate" style={{ ...TYPE.caption, color: nx ? t.sub : t.faint }}>
            ...
            <div className="mb-7" data-tour="family-upcoming" style={{ borderTop: `0.5px solid ${HAIR(t.ink, 0.14)}` }}>
              {rest.map((b) => (
                <div key={b.id} className="flex items-center gap-3 py-3" style={{ borderBottom: `0.5px solid ${HAIR(t.ink, 0.14)}` }}>
                  ...
                  <span className="flex-1 min-w-0 truncate" style={{ ...TYPE.body, color: t.ink }}>{first(nameOf(b.playerId)) || b.who}</span>
                  <span className="shrink-0" style={{ ...TYPE.small, color: b.status === "requested" ? CAUTION : t.sub }}>{b.time}</span>
                </div>
              ))}
            </div>
          </>)}
...
function FamilyKid({ kid, lessons = [], drills = [], bookings = [], canBook, onBook, onMessage, onOpenLesson, onAllLessons, todayIso, pop }) {
  ...
  return (
    <SwipeBack onBack={pop}>
      <Screen title={first(kid.name)} onBack={pop}
              meta={kid.coachName ? `${tr("with")} ${kid.coachName}${kid.sport && SPORTS[kid.sport] ? ` · ${SPORTS[kid.sport].label}` : ""}` : tr("No coach yet")}>
        <div className="px-6">
          <div className="flex gap-2 mb-7">
            <button data-tour="kid-book" onClick={() => { hapticCommit(); onBook && onBook(kid); }} disabled={!canBook} className="flex-1 active:opacity-75 disabled:opacity-35"
                    style={{ minHeight: 48, borderRadius: R.control, background: t.accent, ...TYPE.small, fontWeight: 600, color: t.onAccent }}>
              {canBook ? tr("Book a lesson") : kid.coachId ? tr("No hours set") : tr("Book a lesson")}
            </button>
            <button onClick={() => { haptic(8); onMessage && onMessage(kid); }} disabled={!kid.coachId} className="flex-1 active:opacity-60 disabled:opacity-35"
                    style={{ minHeight: 48, borderRadius: R.control, border: `0.5px solid ${HAIR(t.ink, 0.16)}`, ...TYPE.small, fontWeight: 600, color: t.ink }}>{tr("Message coach")}</button>
          </div>

          <div className="mb-7" style={{ borderTop: `0.5px solid ${HAIR(t.ink, 0.14)}` }}>
            {[[tr("Next"), next ? `${fmtDay(next.date)} · ${next.time}${next.status === "requested" ? ` · ${tr("asked")}` : ""}` : tr("Nothing booked"), !next],
              /* "All done" is praise; nothing set is not the same thing */
              [tr("To practise"), mineDrills.length === 0 ? tr("None set") : todo ? `${todo} ${todo === 1 ? tr("drill") : tr("drills")}` : tr("All done"), mineDrills.length === 0]].map(([lbl, val, quiet]) => (
              <div key={lbl} className="flex items-baseline gap-3 py-3" style={{ borderBottom: `0.5px solid ${HAIR(t.ink, 0.14)}` }}>
                <span className="shrink-0" style={{ width: 88, ...TYPE.eyebrow, fontSize: 8.5, color: t.faint }}>{lbl}</span>
                <span className="flex-1 min-w-0 truncate" style={{ ...TYPE.body, color: quiet ? t.faint : t.ink }}>{val}</span>
              </div>
            ))}
          </div>

          {mine.length > 0 && (<>
            <Eyebrow>{tr("Lessons")}</Eyebrow>
            <Card className="px-5 mb-7">
              {shown.map((l, i) => (
                <button key={l.id} onClick={() => { haptic(7); soft(); onOpenLesson && onOpenLesson(l); }} className="w-full flex items-center gap-3 text-left active:opacity-50"
                        style={{ minHeight: 46, borderBottom: i === shown.length - 1 ? "none" : `0.5px solid ${HAIR(t.ink, 0.12)}` }}>
...
/* PREFERENCES — the durable choices, saved to the profile's row. The
   ones a person changes are rows on Settings (a coach's register rule
...
================================================================== */
function Tile({ children, onPress, accent, className = "", style = {}, delay = 0, tour }) {
  const t = useT();
  const Comp = onPress ? "button" : "div";
  return (
    <Comp data-tour={tour} onClick={onPress ? () => { haptic(7); soft(); onPress(); } : undefined}
...
const TabBar = React.memo(function TabBar({ tabs, activeIdx, theme, dark, onSelect }) {
  ...
  useEffect(() => {
    ...
    const sync = () => {
      ...
      measure();
      paint(S.current.idx * cell(), false);
    };
  ...
  /* THE PILL IS PLACED IN LAYOUT PIXELS, NOT SCREEN PIXELS.
     ...
     it — and the walkthrough renders the whole app inside
     transform: scale(k). A pill offset taken from that rect and applied
     as translate3d is scaled a second time, so it sat short of its tab,
     further out with every tab along. offsetWidth is the untransformed
     layout width, which is the space translate3d actually moves in;
     the scale is kept only to turn a finger's screen movement back into
     layout pixels while dragging. */
  const measure = () => {
    const el = barRef.current;
    if (!el) return;
    const rect = el.getBoundingClientRect();
    S.current.box = rect;
    S.current.layoutW = el.offsetWidth || rect.width;
    S.current.scale = S.current.layoutW ? (rect.width / S.current.layoutW) || 1 : 1;
  };
  const cell = () => ((S.current.layoutW || 0) / tabs.length);
  ...
  const frame = () => {
    ...
    const c = cell();
    const k = S.current.scale || 1;
    /* the finger moves in screen pixels; the pill moves in layout ones */
    const px = Math.max(0, Math.min((S.current.layoutW || 0) - c, (S.current.x - S.current.box.left) / k - c / 2));
    paint(px, false);
  ...
  return (
    <div className="shrink-0 relative z-30" style={{ paddingLeft: tight ? 10 : 16, paddingRight: tight ? 10 : 16, paddingBottom: 24, paddingTop: 4 }}>
      <div ref={(el) => { barRef.current = el; shellRef.current = el; }} data-tour="tabbar"
        ...
        {/* data-tour-pill so the alignment harness can measure it against
...
const Card = ({ children, className = "", style = {}, delay = 0, tour }) => {
  const t = useT();
  return (<div data-tour={tour} className={className} style={{ background: t.surface, borderRadius: R.surface, boxShadow: ELEV.rest,
...
/* AN ICON FOR EVERY THING A COACH WORKS ON
   ...
   The areas are the verified taxonomy in SPORTS[x].focus — this only
...
/* THE TILE
   ...
   A big box: an icon, one word under it, nothing else. A coach hits it
...
function ActTile({ Icon, label, onTap, tone = "quiet", count, on, dot, tour, aria, h = 76 }) {
  ...
  /* a hairline, so nine tiles read as nine things and not one wash */
  const edge = tone === "accent" || on ? "transparent" : HAIR(t.ink, 0.07);
...
function Row({ label, sub, value, valueColor, checked, onToggle, radio, last, chevron, dot, icon, danger, right, tour }) {
  ...
  return (
    <Tag data-tour={tour} onClick={onToggle ? () => { haptic(6); onToggle(); } : undefined}
...
const Avatar = ({ name, size = 44, group, tint, src, bg, fg }) => {
  ...
  const disc = tint || bg || t.wash;
  const mark = tint ? "#fff" : fg || t.sub;
  return (
...
const IconBtn = ({ C, onOpen, label, count, tour }) => {
  const t = useT();
  return (
    <button data-tour={tour} onClick={() => { haptic(6); onOpen(); }} aria-label={label} className="relative active:opacity-50 p-0.5">
...
function FamilyDashboard({ profiles, conns, practice, tips, bookings, activeProfileId, onSwitch, go, push, right, photos = {}, say, counts }) {
  ...
  const Person = ({ x, big }) => {
    ...
    return (
      <button data-tour="family-person" onClick={() => { haptic(8); soft(); onSwitch(p.id); }}
...
/* THE COACH'S DAY
   ...
   Four tiles at the top: log, capture, register, add someone. They are
   the whole job and they are never more than one tap away, whatever
        ...
        {(() => {
          ...
          return (
            <div className="mb-6">
              <TileGrid>
                {tiles.map((x) => (
                  <ActTile key={x.key} tour={x.tour} h={82} Icon={x.Ico} label={x.label} count={x.count}
...
/* THE STAGE RIDES ON THE LESSON

   A player's level, the second axis where the sport has one, and the
   horse a rider was on are tags on the lesson (`subs`), so nothing
   changed in the database: `stageOf()`, `extraOf()` and `mountOf()`
   read them back wherever a lesson is shown. */
...
/* LOG A LESSON
   ...
   Two screens. Who, then everything else on one page.

   The page is tiles: what was worked on across the top as a grid of
   icons, then video, photo and voice as three boxes that open the
   camera itself, then drills and the tip. The level is not asked for —
   it is what the player is already on, shown as a tag and changed only
   if it has changed. Anything captured during the lesson is already
   attached; a swipe to the left takes one off.

   No wrapping rows of pills, no page to reach a note, no question a
   coach has to read twice. */
  ...
  return (
    <button data-tour={tour} aria-label={person.name} aria-pressed={on} onClick={() => { hapticCommit(); soft(); onTap(); }}
      ...
      <Avatar name={person.name} size={40} group={group} src={person.avatarPath ? avatarUrl(person.avatarPath) : undefined}
              bg={on ? "rgba(255,255,255,0.18)" : t.page} fg={on ? "#fff" : t.sub} />
...
function Wizard({ cfg, sport, prefill, groups, captured, setCaptured, onAnnotate, onPublish, onCancel, livePlayers, askReview = true, lessonCounts, onSaveDrill, startView, library, tipPrompts, lastFor, todayIds, todayBookings, liveNow, recent, recentMounts, pinnedToday, onAddPlayer }) {
  ...
  const [pickedGroup, setPickedGroup] = useState(prefGroup ? prefGroup.name : null);
  ...
  /* ---------- the level: what the player is already on ---------- */
  ...
  useEffect(() => {
    ...
    if (whoKey === firstKey.current && lastFor === firstMemory.current) return;
  ...
  /* WHAT WAS CAPTURED DURING THE LESSON IS ALREADY ON IT. A coach who
     filmed two swings mid-lesson should not have to go and fetch them;
     they are attached, and a swipe to the left takes one off. */
  const pulled = [...who.map((r) => r.name), "__unassigned"].flatMap((name) => (captured[name] || [])
    .map((it) => ({ ...it, from: name === "__unassigned" ? null : name })));
  const forget = (ids) => setCaptured((c) => { const out = {}; for (const k of Object.keys(c)) out[k] = (c[k] || []).filter((x) => !ids.has(x.id)); return out; });
  const addPhoto = (kind) => {
    haptic(10); soft();
    const at = Date.now();
  ...
  /* ---------- derived ---------- */
  const firstName = wizFirst((who[0] || {}).name);
  ...
  const whenLine = [dayLabel, prefill && prefill.time ? prefill.time : null].filter(Boolean).join(" · ");
  ...
  const submit = async () => {
    ...
    let ok = false;
    try { ok = (await onPublish(payload())) !== false; } catch (e) { ok = false; /* the toast has said why */ }
    /* what went onto the lesson comes off the clipboard: otherwise the
       next lesson with the same player quietly carries today's clips */
    if (ok && pulled.length) forget(new Set(pulled.map((it) => it.id)));
  ...
  const pickPerson = (pl) => {
    if (adding) { const next = ticked(pl) ? who.filter((r) => r.id !== pl.id) : [...who, pl]; setPickedGroup(keepGroup(next)); setWho(next); haptic(7); return; }
    setWho([pl]); setPickedGroup(null); setQ(""); setView("main");
  };
  const pickGroup = (g) => {
    const mems = membersOf(g);
    if (adding) { setWho([...who, ...mems.filter((m) => !ticked(m))]); setPickedGroup(null); haptic(7); return; }
        ...
        <div key="who" className="flex flex-col h-full relative" style={{ background: t.page }}>
          ...
          <div className="flex-1 overflow-y-auto">
            ...
            <div className="px-6 pb-6">
              ...
              {groupTiles.length > 0 && (
                <>
                  <WizLabel>{tr("Groups")}</WizLabel>
                  <TileGrid>
                    {groupTiles.map((g) => <FaceTile key={g.id || g.name} person={g} group caption={`${membersOf(g).length}`} on={pickedGroup === g.name} onTap={() => pickGroup(g)} />)}
                  </TileGrid>
                </>
              )}
              {rest.length > 0 && (
          ...
          {adding && who.length > 0 && (
            <WizDocked><Button tone="ink" onClick={() => { setAdding(false); setView("main"); }}>{`${tr("Done")} · ${who.length}`}</Button></WizDocked>
          )}
        </div>
      </SwipeBack>
    );
  ...
  /* ---------- the level, changed only when it has changed ---------- */
  if (view === "stage") {
    ...
    const pickRung = (st) => {
      ...
      if (st.input) { const el = document.getElementById(`rung-${st.id}`); if (el) { el.focus(); el.select(); } return; }
    ...
    return (
      <SwipeBack key="stage" onBack={back}>
        <div key="stage" className="flex flex-col h-full relative" style={{ background: t.page }}>
          <WizBar onBack={back} right={<TextBtn color={t.ink} onClick={() => setView("main")}>{tr("Done")}</TextBtn>} />
          <div className="flex-1 overflow-y-auto pb-8">
            <WizHero sub={pickedGroup || firstName}>{tr("Level")}</WizHero>
            <div className="px-6">
              {groupsOf(stages).map((gr, gi) => (
                <div key={gr.group || `_${gi}`}>
                  {gr.group && <WizLabel>{gr.group}</WizLabel>}
                  {gr.items.map((st) => {
                    const on = stage === st.id;
                    return (
                      <div key={st.id} className="flex items-center gap-3" style={{ minHeight: 56, borderBottom: hair }}>
                        ...
                        {st.input ? (
                          <input id={`rung-${st.id}`} value={on ? stageVal : ""} inputMode="decimal" enterKeyHint="done" placeholder={st.hint || ""} aria-label={st.label}
                                 ...
                                 className="outline-none text-right shrink-0" style={{ width: 96, minHeight: 40, padding: "0 10px", borderRadius: R.field,
                                          background: on ? `${t.accent}14` : t.wash, border: `1px solid ${on ? `${t.accent}55` : "transparent"}`,
                                          fontFamily: ui, fontSize: 16, fontWeight: on ? 600 : 400, color: t.ink, fontVariantNumeric: "tabular-nums" }} />
                        ) : (
                          <button onClick={() => pickRung(st)} aria-label={st.label} className="py-2 active:opacity-50"><WizCircle on={on} /></button>
                        )}
                      </div>
                    );
                  })}
  ...
  return (
    <SwipeBack key="main" onBack={onCancel}>
      <div key="main" className="flex flex-col h-full relative" style={{ background: t.page }}>
        <WizBar onBack={onCancel} close />
        <input ref={fileInput} type="file" multiple className="hidden" onChange={(e) => { addFiles(Array.from(e.target.files || [])); e.target.value = ""; }} />
        <div className="flex-1 overflow-y-auto">

          {/* who it was with, when, and the level they are on */}
          <div className="px-6 pt-1" style={{ animation: "fadeUp 420ms cubic-bezier(.22,1,.36,1) both" }}>
            <div className="flex items-center gap-3">
              ...
              <button onClick={() => { haptic(6); setAdding(true); setView("who"); }} aria-label={tr("Add someone")}
                      className="rounded-full flex items-center justify-center shrink-0 active:opacity-60"
                      style={{ width: 38, height: 38, background: t.wash }}><Plus size={17} color={t.ink} strokeWidth={2} /></button>
            </div>
              ...
              <span data-tour="wiz-when" className="relative flex items-center">
                ...
                {needsDate && (
                  <input type="date" aria-label={tr("Date")} value={dateIso} max={maxIso}
                         ...
                         onChange={(e) => { const p = e.target.value.split("-"); if (p.length === 3 && p[0]) { setLogY(Number(p[0])); setLogM(Number(p[1])); setLogD(Number(p[2])); } }}
          ...
          <div className="px-6" style={{ marginTop: SPACE.block }}>
            ...
            {/* everything on the lesson, including what was captured during it */}
            {items.length > 0 && (
              <div className="mt-3" style={{ borderTop: hair }}>
                {items.map((it) => (
                  <SwipeRow key={it.id} deleteLabel={tr("Remove")} onDelete={() => removeItem(it)}>
                    <div className="flex items-center gap-3 px-1" style={{ minHeight: 54, borderBottom: hair, background: t.page }}>
                      {(() => { const K = it.kind === "video" ? Play : it.kind === "voice" ? Mic : it.kind === "note" ? Edit3 : ImageIcon;
                        return <K size={15} color={t.sub} strokeWidth={1.8} />; })()}
                      <span className="flex-1 min-w-0">
                        <span className="block truncate" style={{ ...TYPE.body, color: t.ink }}>
                          {it.kind === "video" ? tr("Video") : it.kind === "voice" ? tr("Voice note") : it.kind === "note" ? (it.text || tr("Note")) : tr("Photo")}
                        </span>
                        {(it.from || it.capturedId != null) && <span className="block truncate" style={{ ...TYPE.caption, color: t.faint }}>{it.from ? `${tr("captured")} · ${wizFirst(it.from)}` : tr("captured earlier")}</span>}
                      </span>
                    </div>
                  </SwipeRow>
                ))}
          ...
          <div className="px-6" style={{ marginTop: SPACE.block }}>
            ...
            {showReviewAsk && (
              <div className="flex items-center gap-3 mt-4" style={{ minHeight: 52, borderTop: hair }}>
                <span className="flex-1" style={{ ...TYPE.body, color: t.ink }}>{tr("Ask")} {firstName} {tr("for a rating")}</span>
...
function CoachRoster({ groups, roster, push, sheet, right, nouns, code, lessonCount = 0, stageFor }) {
  ...
  const row = (key, tour, onTap, avatar, label, sub) => (
    <button key={key} data-tour={tour} onClick={onTap} className="w-full flex items-center gap-3.5 text-left active:opacity-50"
            style={{ minHeight: 64, borderBottom: `0.5px solid ${HAIR(t.ink, 0.1)}` }}>
      {avatar}
      <span className="flex-1 min-w-0">
        ...
        {tab === nounTitle ? (<>
          ...
          <div style={{ borderTop: `0.5px solid ${HAIR(t.ink, 0.1)}` }}>
            {groups.map((g) => row(g.id, undefined, () => { haptic(6); push("group:" + g.name); }, <Avatar name={g.name} size={40} group />, g.name,
                `${g.members.length} ${nouns || "players"} · ${DAY_NAMES[g.day].slice(0, 3)} ${g.time}`))}
            {row("new", "roster-newgroup", () => { hapticCommit(); sheet("group"); },
                <span className="rounded-full flex items-center justify-center shrink-0" style={{ width: 40, height: 40, background: t.wash }}><Plus size={17} color={t.sub} strokeWidth={2} /></span>,
                tr("Create a group"), null)}
          </div>
        )}
      </div>
    </Screen>
      ...
      <Screen title={name} onBack={pop} meta={meta}>
        <div className="px-6 pb-2">
          ...
          <div className="mb-7" data-tour="player-lessons" style={{ borderTop: `0.5px solid ${HAIR(t.ink, 0.14)}` }}>
            ...
            {past.length > 0 && (
              <button data-tour="player-all-lessons" onClick={() => { haptic(7); soft(); onAllLessons ? onAllLessons() : push("history:" + (r.id || name)); }}
          ...
          {(f.tip || seriesFor) && (
            <div className="mb-7">
              ...
              {f.tip && (
                <div className="flex items-baseline gap-3 py-2">
                  <span className="shrink-0" style={{ width: 52, ...TYPE.eyebrow, color: t.faint }}>{tr("Tip")}</span>
...
function PlayerHistory({ name, cfg, attendance, pop, push, say, lessons }) {
  ...
  const total = att ? att.showed + att.noShow : 0;
        ...
        {tab === tr("Attendance") && (
          <div className="px-6 pb-2">
            {!att ? (
              ...
              <p className="py-4" style={{ ...TYPE.body, color: t.ink, borderTop: `0.5px solid ${HAIR(t.ink, 0.12)}`, borderBottom: `0.5px solid ${HAIR(t.ink, 0.12)}` }}>
                {att.showed} {tr("of")} {total} {tr("attended")}{att.noShow ? ` · ${att.noShow} ${tr("missed")}` : ""}
              </p>
            )}
          </div>
        )}
...
function ImportRoster({ close, say, noun, nouns, code }) {
  const t = useT();
  const live = useLive();
  ...
  return (
    <>
      <h2 className="mb-5" style={{ fontFamily: display, fontSize: 24, letterSpacing: "-0.025em", color: t.ink }}>{tr("Invite")} {nouns}</h2>
...
function GroupCreate({ cfg, coachSport, onCreate, close, livePlayers }) {
  ...
  /* rows, so two players called the same thing are two members; the
     harness has no ids, so its rows are { id: name, name } */
  const tog = (p) => { haptic(5); setMembers(members.some((x) => x.id === p.id) ? members.filter((x) => x.id !== p.id) : [...members, p]); };
  const ready = name.trim() && members.length > 0;
  const POOL = (livePlayers ?? PLAYERS).map((p) => (typeof p === "string" ? { id: p, name: p } : p));
  ...
  return (
    <>
      ...
      <div className="flex flex-wrap gap-2 mb-2">
        {shown.map((p) => {
          const on = members.some((x) => x.id === p.id);
          return (
            <button key={p.id} onClick={() => tog(p)} className="rounded-full pl-1.5 pr-3.5 flex items-center gap-2 active:opacity-60"
                    style={{ minHeight: 42, background: on ? t.ink : t.surface, border: `1px solid ${on ? t.ink : t.hair}` }}>
              <Avatar name={p.name} size={30} />
              <span style={{ fontFamily: ui, fontSize: 13.5, fontWeight: 600, color: on ? "#fff" : t.sub }}>{p.name.split(" ")[0]}</span>
      ...
      <Button disabled={!ready} onClick={() => { onCreate({ name: name.trim(), members: members.map((m) => m.name), memberIds: members.map((m) => m.id), day, time, weeks, sport: coachSport }); close(); }}>
    ...
    <Screen title={tr("Practice")} meta={tr("What you've set, and who's doing it")} right={right}>
      ...
      <div className="px-6 pb-2"><Card>{live && rows.length === 0 ? (<div className="p-7 text-center"><p style={{ fontFamily: ui, fontSize: 14, color: t.sub }}>{tr("No one on your roster yet.")}</p></div>) : rows.map((r, i) => { const none = r.total === 0; const isOpen = live && open === r.id; return (
        ...
        <Row label={r.name} sub={none ? "Nothing set" : `${r.done} of ${r.total} done`} icon={<Avatar name={r.name} size={38} />}
             ...
             last={i === rows.length - 1 && !isOpen} onToggle={() => (live ? setOpen(isOpen ? null : r.id) : sheet(r))} />
        {isOpen && (
...
function DrillLibrary({ cfg, sport, library, addDrill, removeDrill, pop, assign, say }) {
  ...
  return (
    <SwipeBack onBack={pop}>
      <Screen title={tr("Drills")} onBack={pop} meta={library.length ? `${library.length} ${tr("in your library")}` : ""}
              right={<TextBtn tour="library-new" onClick={() => { haptic(8); setAdding(!adding); }}>{adding ? "Cancel" : "New"}</TextBtn>}>
        {adding && (
          <div className="px-6 mb-5"><Card className="p-5">
...
function AssignBody({ cfg, library, preset, focusHint, onAssign, onSaveDrill, close, livePlayers }) {
  ...
  return (
    <>
      ...
      <Eyebrow>{library.length ? tr("Or write a new one") : tr("Write one")}</Eyebrow>
  ...
  return (
    <SwipeBack onBack={pop}>
      <Screen title={tr("Availability")} onBack={pop} meta={`${total} slots a week`} right={<TextBtn onClick={() => { setAvail(draft); say("Availability saved"); pop(); }}>{tr("Save")}</TextBtn>}>
        ...
        <div className="px-6 pb-2"><Card tour="avail-days">{DAY_NAMES.map((name, day) => { const times = draft[day] || []; const on = times.length > 0; const expanded = openDay === day; return (
...
function CalendarScreen({ role, conn, avail, blocked, setBlocked, bookings, seedBooked, onBook, onCancel,
  ...
  return (
    <Screen title={forName ? `${tr("Book for")} ${forName}` : L_CAL(role, readOnly)} right={right} meta={role === "coach" ? "" : conn ? `with ${conn.coach}` : ""}>
      ...
      {role === "player" && family && family.length > 0 && (() => {
        ...
        return (
          <div className="px-6 mb-8">
            ...
            <div className="flex gap-2 overflow-x-auto pb-4" data-tour="cal-family" style={{ scrollbarWidth: "none" }}>
      ...
      {noHours && (
        <div className="px-6 mb-4">
          {/* A parent has no coach of their own, but they do book for their
              children — so offer that here rather than sending them away. */}
          {parent && (kids || []).length > 0 ? (
            <>
              <Eyebrow>{tr("Book for")}</Eyebrow>
              <Card className="px-5 mb-2">
                {(kids || []).map((k, i, arr) => (
                  <button key={k.id} onClick={() => { hapticCommit(); soft(); onBookFor && onBookFor(k); }} disabled={!k.canBook}
                          className="w-full flex items-center gap-3 py-3.5 text-left active:opacity-50 disabled:opacity-40"
                          style={{ borderBottom: i === arr.length - 1 ? "none" : `0.5px solid ${HAIR(t.ink, 0.12)}` }}>
                    <Avatar name={k.name} size={34} src={avatarUrl(k.avatarPath)} />
                    <span className="flex-1 min-w-0">
                      <span className="block truncate" style={{ ...TYPE.body, color: t.ink }}>{k.name.split(" ")[0]}</span>
                      <span className="block truncate" style={{ ...TYPE.caption, color: t.faint }}>
            ...
            <p className="py-6 text-center" style={{ fontFamily: ui, fontSize: 14, lineHeight: 1.6, color: t.sub }}>
              {parent ? tr("Nobody's coach has set times yet.") : tr("Your coach hasn't set times yet.")}
            </p>
          )}
        </div>
      )}
...
function ProfileScreen({ account, me, role, avatar, sports, activeSport, onPickSport, onAddSport, onRemoveSport, onSave, onUploadAvatar, onRemoveAvatar, onChangePassword, onSignOut, onDelete, pop, push, say }) {
  ...
  const save = async () => {
    ...
    hapticCommit();
    setBusy(true); setErr("");
    /* a coach's sport is what their code was handed out under — it is
       never written from here */
    const res = await onSave({ name: f.name, ...(role === "coach" ? { club: f.club } : { sport: f.sport }), dateOfBirth: f.dob || null, phone: f.phone, bio: f.bio });
    setBusy(false);
    if (res && res.error) { hapticWarn(); setErr(res.error.message || tr("Couldn't save that.")); return; }
    say(tr("Saved"));
  };
...
function InviteBody({ code, say }) {
  ...
  return (
    <div className="flex flex-col items-center">
      ...
      <Button onClick={share} disabled={!code}><span className="flex items-center justify-center gap-2"><Share2 size={17} /> {tr("Share invite")}</span></Button>
...
================================================================== */
function SearchScreen({ role, cfg, library, tips, pop, go, push, lessons: given, people: livePeople, threads: liveThreads }) {
...
export default function Nosca({ demo: demoProp, account, onSignOut, data, onJoinCoach, onProfileChanged, showcase, invite, onInviteUsed } = {}) {
  /* A real account gates every seed generator. The answer travels by
     ...
     answer rather than flipping its host's. */
  const live = !!account;
  /* `showcase` renders one state of the product — inert, seeded, at
     390×780 — for the walkthrough. Everything it names arrives as lazy
     initial state, so the frame paints the right screen on its first
     frame with no flash and no effects. Nothing else changes for a real
     account or for the ?demo harness. */
  const sc = showcase || null;
  const persona = sc ? showcasePersona(sc) : null;
  ...
  const demo = sc ? false : demoProp !== undefined ? demoProp
    : (typeof window !== "undefined" && window.location.search.includes("demo"));

  useTypefaces();
  const [flow, setFlow] = useState(sc ? "app" : "role");
  ...
  useEffect(() => {
    ...
    setFlow("app");
    /* where the app opens: the person's own choice if they made one and
       it is still a tab they have, otherwise the natural home */
    const natural = account.role === "coach" ? "today" : account.accountType === "parent" ? "family" : "home";
    setStack([startOn && startOn !== "auto" ? startOn : natural]);
  ...
  useEffect(() => {
    ...
    const p = data.prefs;
    /* An optimistic savePrefs writes only the fields it was given, so
       the object here is often partial — every read falls back to the
    ...
    setPrefsLocal({
      ...
      attendance: p.attendance ?? PREF_DEFAULTS.attendance,
      showRecord: p.show_record ?? PREF_DEFAULTS.showRecord,
      showComps: p.show_comps ?? PREF_DEFAULTS.showComps,
  ...
  /* what is still ahead: something that has already happened is not a
     countdown, and left in it sat at "0 days" on Today for ever */
  const liveEvents = data ? (liveComps || []).filter((c) => !c.past).map((c) => ({ ...c, when: c.date })).sort((a, b) => a.days - b.days) : [];
  /* the Ahead screen keeps the ones that have been and gone, under the rest */
  const allEvents = data ? [...liveEvents, ...(liveComps || []).filter((c) => c.past).map((c) => ({ ...c, when: c.date })).sort((a, b) => b.days - a.days)] : [];
  ...
  const [peek, setPeek] = useState(sc ? (sc.peek || null) : null);
  ...
  useEffect(() => {
    if (sc) return;
    try {
      if (window.sessionStorage.getItem("nosca.tour.now") === "1") {
        window.sessionStorage.removeItem("nosca.tour.now");
        setTour(true);
      }
  ...
  /* Exactly the rows a call-off will touch on a given day: confirmed
     lessons AND requests still waiting on an answer, since calling the
     day off cancels those too. The sheet must not list fewer. */
  const callOffRows = (day) => {
  ...
  /* Gated at the initialiser, the way `series`, `requests` and the
     seeded registers already are — a real coach was being shown an
  ...
  /* The sports' starter drills are the harness's, not a coach's work.
     A real account starts with an empty library and fills it from its
     own saved drills (custom_drills, layered in below). */
  const [library, setLibrary] = useState(() => Object.fromEntries(Object.entries(SPORTS).map(([id, cf]) => [id, account ? [] : cf.drills])));
  ...
  const saveSeries = async (v) => {
    if (data) {
      /* the person the coach picked, by id. Falling back to the name
         is only for the calendar's "new arrangement" path, which has
         nobody chosen and defaults to the first on the roster. */
      const who = (recurFor && recurFor.id ? (data.roster || []).find((r) => r.id === recurFor.id) : null)
        || (data.roster || []).find((r) => r.name === v.who);
      if (!who) { hapticWarn(); say(tr("Pick someone on your roster.")); return; }
      const old = (data.recurring || []).find((x) => x.playerId === who.id);
  ...
  /* THE COACH'S name — for a coach that is themselves, for a player the
     ...
     every player's account show their coach's name and initials. */
  ...
  /* the same rule as playerLessons: a parent's headline is theirs, not
     the one their child's coach set for the child */
  const myTips = data ? (role === "coach" ? data.tips : (data.tips || []).filter((tp) => !account || tp.playerId === account.id)) : freshAccount ? [] : (tips[pKey] || []);
  ...
  /* Who was logged most recently (five), and which horses, for the log's
     Who page and its Level page. The harness orders its roster by `last`. */
  ...
  const publish = async (l) => {
...

    {
      ...
      const named = names[0];
      /* BY ID. The wizard hands back the ids of the people who were
         ticked; resolving the display name against the roster instead
         gave the lesson to whichever of two players with the same name
         happened to be listed first. The name lookup survives only as
         the fallback for a lesson that reached here without ids. */
      const pickedIds = (Array.isArray(l.whoIds) ? l.whoIds : []).filter(Boolean);
      const match = pickedIds.length
        ? (data.roster || []).find((r) => r.id === pickedIds[0])
        : (data.roster || []).find((r) => r.name === named);
  ...
  /* WHO DID THEY MEAN. Callers hand over whatever they hold — a roster
     row, an id, or a display name — and this settles it. A name is the
     last resort, because two players called the same thing resolve to
     whichever the roster lists first; anything that knows an id passes
     one, and the harness has no ids, so a name is all it ever has. */
  /* a roster row by its id, or by name from an older entry point */
  const byKey = (key) => { const list = (data && data.roster) || []; return list.find((r) => r.id === key) || list.find((r) => r.name === key) || null; };
  const personOf = (v) => {
    if (!v) return null;
    const list = (data && data.roster) || [];
    if (typeof v === "object") return v.id ? v : (list.find((r) => r.name === v.name) || v);
    return list.find((r) => r.id === v) || list.find((r) => r.name === v) || { id: v, name: v };
  };
  const openAssignDrills = (who, focusId) => { setAssignTo(personOf(who)); setAssignFocus(focusId); setSheet("assign"); haptic(8); };
  const saveDrill = (d) => {
    setLibrary((l) => {
  ...
  const openAssignTip = (who, focusLabel) => { setAssignTo(personOf(who)); setAssignFocus(focusLabel); setSheet("tip"); haptic(8); };
  const doSetTip = async (tip) => {
    const person = personOf(assignTo);
    const assignName = person ? person.name : "";
    if (data) {
      const who = (data.roster || []).find((r) => r.id === (person || {}).id);
      if (!who) { hapticWarn(); say(tr("Pick someone on your roster.")); return; }
      ...
      chime(); say(`${tr("Focus set for")} ${assignName.split(" ")[0]}`); haptic(18); return;
    }
    if (assignName === "Marcus Tran" && coachSport === "golf") {
      setTips((p) => ({ ...p, "1:golf": [{ id: Date.now(), title: tip.title, body: tip.body, focus: assignFocus || "General", date: "Today", weeksAgo: 0 }, ...(p["1:golf"] || [])] }));
    }
    chime(); say(`Tip sent to ${assignName.split(" ")[0]}`); haptic(18);
  };
  ...
  const createGroup = async (g) => {
    if (data) {
      /* the picker hands over ids; resolving the names again would
         merge two players called the same thing into one member */
      const byName = Object.fromEntries((data.roster || []).map((r) => [r.name, r.id]));
      const ids = (g.memberIds && g.memberIds.length ? g.memberIds : g.members.map((n) => byName[n])).filter(Boolean);
      const rec = { id: `g-${Date.now()}`, name: g.name, members: g.members, memberIds: ids,
  ...
  const playerLessons = (() => {
    /* Row-level security hands an adult their family's lessons as well
  ...
  /* A real coach's own numbers, from their real lessons: the last
     seven days, and everything since 1 January. No hours — a lesson row
     carries no length, so any figure would be a guess. */
  const liveStats = data ? (() => {
    ...
    return { weekDone: week, seasonDone: season };
  })() : null;
  ...
  /* THE HEADER IS YOURS. Your name, your picture, your settings —
     whoever else is in your family. The family had taken this spot and
     with it the only way into a person's own account; it is a tab of
     its own now, and only once there is a family to open. */
  /* One control for the person, not two. A real account gets a single
     named pill — face, first name, chevron — because a plain avatar
     beside a pill carrying the same face and the same destination was
     the header saying everything twice. The harness keeps its profile
     switcher on the left and its avatar on the right. */
  ...
  /* In a family at all — which is what earns the tab. Having someone to
     look after (hasFamily) is a different question, and the parent's
     landing screen turns on that one. */
  const inFamily = data ? !!data.family : profiles.some((pf) => pf.age);
  const familyTab = { id: "family", icon: Users, label: tr("Family") };
  ...
  /* A coach searches their own drill library; anyone else searches the
    ...
    const hname = hp ? hp.name : hkey;
    /* a real record: what the coach actually marked for this person,
       by id — a register's marks are keyed by the player's id */
    const realAtt = data ? (() => {
      let showed = 0, noShow = 0;
      if (hp) Object.values(registers || {}).forEach((reg) => { const v = reg[hp.id]; if (v === "in") showed++; else if (v === "out") noShow++; });
      return showed + noShow ? { [hname]: { showed, noShow } } : {};
    })() : null;
    /* their private lessons and the group sessions they were marked at —
    ...
    body = <AtRisk list={atRisk(roster, mySeriesLive, live, bookedAhead)} pop={pop} onMessage={(k) => push("thread:" + k)} />;
    ...
    body = <FamilyScreen family={data ? data.family : (sc && sc.parent ? showcaseFamily : null)} isJunior={juvenile} live={!!data}
                         onCreate={data ? (n) => data.createFamily(n) : async () => ({})}
                         onJoin={data ? async (c) => {
                           const r = await data.joinFamily(c);
                           if (r && !r.error) { setCeleb({ label: tr("Joined"), sub: (r.family && r.family.name) || tr("your family") }); go("family"); }
                           return r;
                         } : async () => ({})}
                         onLeave={data ? () => data.leaveFamily() : async () => ({})} onRename={data ? (n) => data.renameFamily(n) : async () => ({})}
                         lookup={data ? data.lookupCode : null} say={say} pop={pop} />;
    ...
    const ang = screen.slice("annotate:".length);
  ...
  } else if ((screen === "family" || screen.startsWith("familyKid")) && (data || sc)) {
    ...
    const kid = kids[0];
    const famLessons = data ? (data.lessons || []) : (kid ? (playerLessons || []).slice(0, 3).map((l) => ({ ...l, playerId: kid.id })) : []);
    const famDrills = data ? (data.drills || []) : (kid ? (practice["2:tennis"] || []).map((d) => ({ ...d, playerId: kid.id })) : []);
    const famBookings = data ? (liveBookingRows || []) : (kid ? [{ id: 1, playerId: kid.id, date: isoOf(7, 31), time: "4:30 pm", status: "confirmed" },
                                                                 { id: 2, playerId: kid.id, date: isoOf(8, 6), time: "5:00 pm", status: "confirmed" }] : []);
    const famHours = data ? (data.hoursByPlayer || {}) : (kid ? { [kid.id]: { days: DEFAULT_AVAIL.tennis } } : {});
    const iso = isoOf(todayMD.m, todayMD.d);
    if (screen.startsWith("familyKid")) {
      /* "first" is what the walkthrough asks for; a real tap passes the id */
      const want = screen.slice("familyKid:".length);
      const k = want === "first" ? kid : kids.find((x) => String(x.id) === want) || kid;
      ...
      body = <FamilyHome family={fam} isJunior={juvenile} dependants={juvenile ? [] : kids}
                         ...
                         onOpenKid={(k) => push("familyKid:" + k.id)} onSettings={() => push("familyCode")} onSetUp={() => push("familyCode")}
                         right={navRight} />;
    }
            ...
            : sheet === "family" ? <FamilySheet profiles={profiles} activeProfileId={activeProfileId} onSwitchProfile={switchProfile} onAddChild={addChild} conns={conns} activeConnId={activeId} onPickConn={(id) => { setActiveId(id); go("home"); }} onAddConn={addConn} onViewGroups={() => push("groups")} onFamily={() => push("familyCode")} onPhoto={() => setSheet("photo")}
              ...
              : sheet === "pickRecurWho" ? (
                  <>
                    ...
                    <div className="flex flex-col gap-2">
                      {roster.map((r, i) => {
                        const has = mySeriesLive.some((x) => (r.id ? x.playerId === r.id : x.who === r.name) && !x.ended);
                        return (
                          <Tile key={r.id || r.name} className="px-4 py-3.5" delay={i * 45}
                                onPress={has ? null : () => { setRecurFor(r); setSheet("recurring"); }}>
              ...
              : sheet === "recurring" ? <RecurringSetup name={(recurFor || roster[0] || (data ? {} : ROSTER[0]) || {}).name || ""}
                                          existing={data ? (recurFor && mySeries.find((x) => (recurFor.id ? x.playerId === recurFor.id : x.who === recurFor.name))) : series.find((x) => recurFor && x.who === recurFor.name && x.sport === coachSport)}
...
function YouAvatarBtn({ name, src, tint, onOpen, tour }) {
  return (
    <button data-tour={tour} onClick={() => { haptic(6); onOpen(); }} aria-label={tr("Your profile")} title={name || tr("Your profile")} className="active:opacity-50">
      <Avatar name={name} size={30} src={src} tint={tint} />
    </button>
  );
}
